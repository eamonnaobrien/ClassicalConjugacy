...
// Returns the list of semisimple conjugacy classes of Omega(n,F), 
// n and |F| odd
// E class is identified with a triple < m,n,X >,
// where X representative of the class in Omega(n,F), 
...
// At the end of the construction, a block of dimension 1 of el.div. t-1 is added
// Computations for class size are the same for GO(n,F)
// (since x~y in GO iff x~y in Omega for every x,y semisimple)

OmegaClasses:=function(n,F)
   ...
   m:=n div 2;
   q:=#F;
   // indicator of type for compute type of el.div. t+1 or t-1
   indtype:=0;
   if q mod 4 eq 3 then
      indtype:=1;
   end if;
   // S[i] contains all elementary divisors of dimension (2*i) (pp* or p = p*)
   ...
   if n gt 4 then
      Append(~S[1],<0,x,y,1,1,0>);
      Append(~S[1],<0,-x,y,1,2,indtype>);
   end if;
   ...
   for i in [1..n div 4] do
      ...
      for x in X do
         ...
         if x[2] eq 1 then
            ...
            Append(~S[i div 2],<2,C,B,i,Order(C),SpinorNorm(GL(i,F)!C,B)>);
         else
            f:=x[1];
            C:=CompanionMatrix(f);
            ord:=Order(C);
            ...
            Append(~S[i],<1,C,B,i,ord,SpinorNorm(GL(2*i,F)!C,B)>);
         end if;
      end for;
   end for;
   ...
   for P in Omega do
      ...
      for c in C do
         ...
         if #c[1] ge 1 and c[1][1][1] eq 1 then   // i.e. if t-1 was already an el.div.
            ...
            // SpinorNorm can be corrected iff t+1 is an el.div.
            elif #c[1] ge 2 and c[1][2][1] eq 2 then   // i.e. if t+1 is el.div.
            ...
            if IsEven(SN) then
               ...
               Append(~ConjClasses, <order, Integers()!card, Gr!(M^-1*Repr*M)>);
            // SpinorNorm can be corrected iff t+1 is an el.div.
            elif #c[1] ge 1 and c[1][1][1] eq 2 then   // i.e. if t+1 is el.div.
   ...
   for i in [m div 2 +1..m] do
      ...
      if i eq 1 then                        
         Y:={{@x, ConjPol(x) @}: x in AllIrreduciblePolynomials(F,i) | 
                    x ne t and x ne t+1 and x ne t-1};
         Append(~T,<0,IdentityMatrix(F,2),DiagonalMatrix(F,[-1,1]),1,1,0>);
         Append(~T,<0,-IdentityMatrix(F,2),DiagonalMatrix(F,[-1,1]),1,2,indtype>);
      else
         Y:={{@x, ConjPol(x) @}: x in AllIrreduciblePolynomials(F,i)};
      end if;
      X:=[<a[1],#a>: a in Y];
      for x in X do
         if x[2] eq 2 then
            ...
            Append(~T,<1,C,B,i,ord,SpinorNorm(GL(2*i,F)!C,B)>);
         end if;
      end for;
      for L in CartesianPower(F,i) do
         ...
         if IsIrreducible(f) then
            ...
            B:=SymmetricMatrix(F,d);
            Append(~T,<2,C,B,2*i,Order(C),SpinorNorm(GL(2*i,F)!C,B)>);
	 end if;
      end for;
               ...
               for x in T do
                  if x[1] eq 0 and x[5] eq 1 then           // i.e. if the el.div. is t-1
                     ...
                     if IsEven(x[6]) then
                        M:=TransformForm(Form,"orthogonal": Restore:=false);
                        Append(~ConjClasses, <order, Integers()!card, Gr!(M^-1*Repr*M)>);
                     elif x[1] eq 0 and x[5] eq 2 then      // i.e. if the el.div. is t+1
                        Form[2,2]:=-w;
                        card*:=FactCard[1][2];
                        card/:=FactCard[2][2];
                        M:=TransformForm(Form,"orthogonal": Restore:=false);
