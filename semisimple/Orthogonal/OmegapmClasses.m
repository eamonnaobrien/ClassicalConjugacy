...
// Return the list of semisimple conjugacy classes of Omega(n,F), n even, |F| odd
// A class is identified with a triple < m,n,X >, with X representative of the class in Omega(n,F),
...
// Classes having no t-1 or t+1 as el.div. split in two classes in Omega, so the size is divided by 2

OmegapmClasses:=function(n,F,type)
   ...
   if type ne "plus" and type ne "minus" then
   ...
   // Omega(2,q) is abelian, so it's just sufficient to list the elements of Omega(2,q)
   if n eq 2 then
      if type eq "plus" then
         for x in OmegaPlus(2,F) do
            Append(~ConjClasses,<Order(x),1,x>);
         end for;
      else
         for x in OmegaMinus(2,F) do
            Append(~ConjClasses,<Order(x),1,x>);
         end for;
      end if;
      return ConjClasses;
   ...
   //begin general case
   m1:=m div 2;
   // Sp[i] contains all elementary divisors of dimension (2*i) and type "orthogonalplus"
   // for x in Sp[i]:
   ...
   for i in [1..m1] do
      ...
      for x in X do
         if x[2] eq 1 then
            ...
            Append(~Sm[i div 2],<2,C,B,i,Order(C),SpinorNorm(GL(i,F)!C,B)>);
            MultSm[i div 2]+:=1;
         else
            f:=x[1];
            C:=CompanionMatrix(f);
            ...
            Append(~Sp[i],<1,C,B,i,ord,SpinorNorm(GL(2*i,F)!C,B)>);
            MultSp[i]+:=1;
         end if;
      end for;
   end for;
   ...
   for i in [m1+2-(m1 mod 2)..m by 2] do
      ...
      for L in R do
         ...
         if IsIrreducible(f) then
            ...
            B:=SymmetricMatrix(F,d);
            //end construction matrix of the form
            Append(~Sm[k],<2,C,B,i,Order(C),SpinorNorm(GL(i,F)!C,B)>);
            MultSm[k]+:=1;
         end if;
      end for;
   end for;

   // FIRST PART: classes with el.div. of degree less or equal than m
   ...
   for P1 in Omega do
      for mult in [1..m1] do
         ...
         // if a_j <= MultSp[i] a_j-th el.div. of Sp[i], 
         ...
         for c in C do
            ...
            //last part: adding elementary divisors t+1 and t-1 and correct sign
            ...
            // mult*indtype+ind = SpinorNorm of the (t+1)-block
            if IsEven(SN+mult*indtype+ind) then
               Append(~ConjClasses,<order,Integers()!card1,Gr!(M1^-1*DiagonalJoin(Repr,-Suss)*M1)>);
            end if;
      ...
      // because it cannot be corrected by the el.div. t-1, t+1 or an el.div. of degree >m
      P:=P1;
      ...
      for i in [start..&+Mult by 2] do
         for S in Subsets(set,i) do
            ...
            for c in C do
               if c[1][1] eq 0 or MultSp[1] ne 0 then        // need this check if F=GF(3)
                  R1:=<>;
                  for j in [1..m1] do
                     Shelp:=[];
                     ni:=MultSp[j];
                  ...
                  // cp[i] = [<a_j,b_j>: j]; means the a_j-th el.div. of Sp[i] has multiplicity b_j
                  // cm[i] = [<a_j,b_j>: j]; means the a_j-th el.div. of Sm[i] has multiplicity b_j
                  for cp in Cp do
                     for cm in Cm do
                        // cat [0] because it could be empty
                        SN:=&+[&+([0] cat [Sp[j][x[1]][6]*x[2]: x in cp[j]]): j in [1..m1]];
                        SN+:=&+[&+([0] cat [Sm[j][x[1]][6]*x[2]: x in cm[j]]): j in [1..m1]];
                        if IsEven(SN) then                    // i.e. if SpinorNorm =0
                           Form:=Random(MatrixAlgebra(F,0));
                           Repr:=Random(MatrixAlgebra(F,0));
                           card:=Card;
                           order:=1;
   ...
   for i in [m1+1..m-1] do
      // Tp contains elementary divisors of dimension 2*i type pp*, p ne p*
      // Tm contains elementary divisors of dimension 2*i type p=p*
      Tp:=[];
      Tm:=[];
      if i eq 1 then                        
         Y:={{@x, ConjPol(x) @}: x in AllIrreduciblePolynomials(F,i) |
      ...
      for x in X do
         ...
         if x[2] eq 2 then
            ...
            C:=DiagonalJoin(C,Transpose(C^-1));
            B:=BlockMatrix(2,2,[0,IdentityMatrix(F,i),IdentityMatrix(F,i),0]);
            Append(~Tp,<1,C,B,i,ord,SpinorNorm(GL(2*i,F)!C,B)>);
         end if;
      end for;
      ...
      for L in R do
         ...
         if IsIrreducible(f) then
            ...
            B:=SymmetricMatrix(F,d);
            Append(~Tm,<2,C,B,2*i,Order(C),SpinorNorm(GL(2*i,F)!C,B)>);
 	 end if;
      end for;
      ...
      for P1 in Omega do
            ...
            if mult in P1 then
               ...
               for c in C do
                  ...
                  for x in Tp do
                     ...
                     if IsEven(SN1+mult*indtype+ind) then  // mult*indtype+ind = SpinorNorm of the (t+1)-block
                        Append(~ConjClasses,<order1,Integers()!card2,Gr!(M1^-1*DiagonalJoin(Repr1,-Suss)*M1)>);
                     end if;
                     for v in [1..mult-1] do
                        Suss[2*v-1,2*v-1]:=-1;
         ...
         // c[i] = [<a_j,b_j>: j]; means the a_j-th el.div. of dimension i has multiplicity b_j
         // a_j-th el.div. of Sp[i] if a_j <= MultSp[i], otherwise is the (a_j-MultSp[i])-th el. div. of Sm[i]
         C:=CartesianProduct(R);
         for c in C do
            sign:=1;
            order:=1;
            ...
            // This must be only of sign + or -, according to the sign of el.div. added before
            if (sign eq -1 and type eq "plus") or (sign eq 1 and type eq "minus") then
               card1:=card/Factorization(q^(Tm[1][4] div 2)+1);
               // card1 is the same for every element in Tm
               card1/:=SequenceToFactorization([<2,1>]);
               for x in Tm do
                  if IsEven(SN+x[6]) then
                     Form1:=DiagonalJoin(Form,x[3]);
                     Repr1:=DiagonalJoin(Repr,x[2]);
               ...
               card1:=card/Factorization(q^(Tp[1][4])-1);   // it's the same for every element in Tp
               card1/:=SequenceToFactorization([<2,1>]);
               for x in Tp do
                  if IsEven(SN+x[6]) then
                     Form1:=DiagonalJoin(Form,x[3]);
            ...
            // The sign is corrected according to the sign of el.div. added before
            ...
            order:=Lcm(2,order);      // 2 = order of el.div. t+1
            if IsEven(SN+indtype*i+ind) then        // i*indtype+ind = SpinorNorm of the (t+1)-block
               Append(~ConjClasses,<order,Integers()!card1,Gr!(M1^-1*DiagonalJoin(Repr,-Suss)*M1)>);
            end if;
            for v in [1..i-1] do
               Suss[2*v-1,2*v-1]:=-1;
   ...
   if type eq "plus" then
	 ...
	 if x[2] eq 2 then
            ...
            if SpinorNorm(Gr!C,B) eq 0 then
