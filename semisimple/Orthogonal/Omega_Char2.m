...
// Return the list of semisimple conjugacy classes of Omega(n,F), 
// for n even, |F| even
// In all comments, el.div. = elementary divisors
// A class is identified with a triple < m,n,X >, 
// with X representative of the class in Omega(n,F),
// m = Order(X), n = cardinality of the class
// Computations for the size of a class are the same for GO.
// Classes having no t-1 as el.div. split in two classes in Omega, 
// so the size is divided by 2

OmegaClasses_Char2:=function(n,F,type)
   ...
   // Omega(2,q) is abelian, ok to list every element of Omega(2,q)
   if n eq 2 then
      if type eq "plus" then
   ...
   // x[2] = its matrix, x[3] matrix of the quadratic form
   ...
   // construction element z not in Omega(n,F):
   // this is for conjugacy classes in GO splitting in more classes in Omega
   B:=DiagonalJoin([FormPlus: i in [1..m]]);
   ...
   z[n-1,n-1]:=0;z[n,n]:=0;
   z[n-1,n]:=1;z[n,n-1]:=1;
   M:=TransformForm(B,type1: Restore:=false);
   z:=M^-1*z*M;
   // SpinorNorm(z) = 1
   ...
   for P1 in Omega do
      for mult in [1..m1] do
         ...
         for c in C do
            SN:=0;
            for i in [1..m1] do         // computation of SpinorNorm
            for x in c[i] do
               if x[1] le MultSp[i] then
                  SN+:=Sp[i][x[1]][6]*x[2];
               else
                  SN+:=Sm[i][x[1]-MultSp[i]][6]*x[2];
               end if;
            end for;
            end for;
            if IsEven(SN) then
      ...
      for i in [start..&+Mult by 2] do
         for S in Subsets(set,i) do
            ...
            // where a+_i = # of el. div. of degree 2i and sign + (same a-_i)
         
            for c in C do
               // need this if F=GF(2)
               if not (c[1][1] eq 0 or MultSp[1] ne 0) then continue; end if;  
   ...
   for i in [m1+1..m-1] do
      ...
      // First part: one of the el.div. of degree <m is t+1
      // In such a case, the sign is corrected when the el.div. t+1 is added
      m2:=m-i;
   ...
   if type eq "plus" then
	 ...
	 if x[2] eq 2 then
            ...
            if SpinorNorm(GL(n,F)!C,B) eq 0 then
               M:=TransformForm(B,"orthogonalplus": Restore:=false);
               card:=Card/(SequenceToFactorization([<2,1>])*Factorization(q^m-1));
