...
// odd block size -- construct generators for Omega 
Omega_Case2 := function (r, f, q, N, a, beta: OrderOnly := false, Verify := false)
   ...
   if Nrows (K) eq 1 then 
      ...
      if type eq "plus" then
         if Verify then assert ClassicalType (G) eq "orthogonalplus"; end if;
         order := #GOPlus (Nrows (K), BaseRing (K));
      elif type eq "minus" then
         if Verify then assert ClassicalType (G) eq "orthogonalminus"; end if;
...
OmegaSSGenerators_Odd := function (r, f, VBeta, N, T, q: OrderOnly := false)
   Gens := [];
   order := 1;
   W := T[1]; 
   for a in Set (W) do
      d := #[x: x in W | x eq a];
      X, ord := Omega_Case1 (r, q, N, a, d: OrderOnly := OrderOnly);
      order *:= ord;
      Gens cat:= X;
   end for;
   ...
   for a in values_V do 
      ...
      X, ord := Omega_Case2 (r, f, q, N, (a - 1) div 2, beta[#beta]: 
          OrderOnly := OrderOnly);
      order *:= ord;
      Gens cat:= X;
   end for;
...
OmegaProcessOneBlock := function (g, form, a: GLO := false)
   if a eq 0 then 
...
OmegaIdentifyParameter := function (g, form: GLO := false)
   F := BaseRing (g);
   ...
   a := odd[1] div 2;
   val, small_g, small_form := OmegaProcessOneBlock (g, form, a: GLO := GLO);
...
OrthogonalParameters := function (g, form: GLO := false)
   ...
   small_g := g; small_form := form;
   for i in [1..#odd] do
      val[i], small_g, small_form := OmegaIdentifyParameter (small_g, small_form: GLO := GLO);
   end for;
...
OmegaTriple := function (d, q, i, j, k, p1, beta: second := false)
   F := GF (q);
...
// matrix generators for Q 
OmegaQuad := function (d, q, i, j, k, l, p1, p2: second := false)
   F := GF (q);
   lambda := PrimitiveElement (F);
   e := Degree (F);
   MA := MatrixAlgebra (GF (q), d);
...
Omega_V_beta := function (k, q, beta)
   d := 2 * k + 1;
...
Omega_V_beta_form := function (k, q, beta)
   d := 2 * k + 1;
   M := MatrixAlgebra (GF(q), d);
   F := Zero (M);
   F[1, d] := (-1)^k;
...
OmegaJordanBlock := function (k, q: beta := 1) 
   if IsOdd (k) then
      return Omega_V_beta ( (k - 1) div 2, q, beta);
   else
      return Omega_W_rep ((k) div 2, q);
   end if;
end function;

OmegaForm := function (k, q, wt: beta := 1)
   if IsOdd (k) then
      return Omega_V_beta_form ((k - 1) div 2, q, beta);
   else
      return Omega_W_form (k div 2, q);
   end if;
end function;

Omega_ClassRep := function (m, q, beta)
   for i in [1..#m] do
   ...
   assert #beta eq #L;
   rep := DiagonalJoin (<OmegaJordanBlock (L[i], q: beta := beta[i]): i  in [1..#L]>);
   form := DiagonalJoin (<OmegaForm (L[i], q, wt: beta := beta[i]): i in [1..#L]>);
...
Omega_QSubgroup := function (a, q, form)
   ...
   for triple in T do 
       ...
       p1 := full[i]; p2 := full[j];
       Append (~L1, 
          OmegaTriple (d, q, i, j, z, p1, beta[Position (zeros, z)]: second := i in second));
   end for;
   L1 := &cat (L1);
   ...
   for pair in I do 
      ...
      if IsEven (p1) and IsEven (p2) then 
         if p2 ne 0 and p2 le Abs (p1) then 
            Append (~L, OmegaQuad (d, q, i, j, k, ell, p1, p2));
            Exclude (~R, pair);
         end if;
         ...
         Append (~L, OmegaQuad (d, q, i, j, k, ell, p1, p2: second := true));
         Exclude (~R, pair);
         ...
         Append (~L, OmegaQuad (d, q, i, j, k, ell, p1, p2: second := false));
         Exclude (~R, pair);
...
OmegaCompleteLabels := function (m) 
...
Omega_ConvertToSpecialRep := function (g, beta)
   G := Parent (g);
   F := BaseRing (G);
   q := #F;
   J, CB, b := JordanForm (g);
   ...
   m := MySort (m);
   r, f, a := Omega_ClassRep (m, q, beta);
   J1, CB1 := JordanForm (r);
   cb := Generic (G) ! (CB1^-1 * CB);
   assert cb * g * cb^-1 eq r;
   Wt, N, T := OmegaCompleteLabels (m);
...
OrthogonalUnipotentCentraliser_Odd := function (G, g, form: 
   ...
   r, f, CB, m, W, N, T := Omega_ConvertToSpecialRep (g, Reverse (beta));
   assert g^(CB^-1) eq r;
   ...
   Add, order := OmegaSSGenerators_Odd (r, f, VBeta, N, T, q);
