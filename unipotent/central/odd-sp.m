...
SpSSGenerators_Odd := function (r, f, VBeta, N, T, q: OrderOnly := false)
   Gens := [];
   order := 1;
...
Sp_QSubgroup_Odd := function (a, q)
   ...
   index := [i : i in [1..#full] | IsEven (full[i])];
   ...
   for j in [1..d] do 
      ...
      if wj lt 0 then 
         for k in [j + 1..d] do 
            wk := full[k];
            if wk lt Abs (wj) then Append (~I, <j, k>); end if;
          end for;
      end if;
   end for;
   ...
   for pair in I do 
      ...
      // "pair is ", pair;
      if IsOdd (p1) then 
         Append (~L, SpQuad (d, q, i, j, k, ell, p1, p2: second := j in second));
         Exclude (~R, pair);
      end if;

      if IsEven (p1) and IsOdd (p2) then 
         Append (~L, SpQuad (d, q, i, j, k, ell, p1, p2: second := i in second));
         Exclude (~R, pair);
      end if;

      if IsEven (p1) and i in first and j in first cat second then 
         Append (~L, SpQuad (d, q, i, j, k, ell, p1, p2: second := j in second));
         Exclude (~R, pair);
      end if;

      if IsEven (p1) and i in second and j in first then 
         Append (~L, SpQuad (d, q, i, j, k, ell, p1, p2: second := true));
         Exclude (~R, pair);
      end if;

      if IsEven (p1) and i in second and j in second then 
...
RestrictToOnePart := function (g, form, k: Perp := false)
   ...
   // V := SymplecticSpace (MatrixAlgebra (F, d) ! form);
   CS := V; 
   for i in [1..k] do 
...
SpUnipotentCentraliser_Odd := function (G, g, form: OrderOnly := false, Verify := false) 
   ...
   Add, order := SpSSGenerators_Odd (r, f, VBeta, N, T, q: OrderOnly := OrderOnly);
