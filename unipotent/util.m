...
ConstructGeneratingSet := function (Q, CP, tau)      
   rank := FrattiniQuotientRank (CP);
   S := sub<CP | [CP.i: i in [1..rank]]>;
   assert NPCgens (S) eq NPCgens (CP);
   C := sub<Generic (Q) | [CP.i @ tau : i in [1..rank]]>;
   return C;
end function;
