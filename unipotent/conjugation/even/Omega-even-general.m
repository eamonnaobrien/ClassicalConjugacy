...
import "../../util.m": ChooseAlpha, FixesQuadraticForm; // MyDiagonalJoin, MySort;
...
import "Omega-W-beta.m": Omega_WBeta_ConjugationMatrix;
import "Sp-even-general.m": Sp_Even_Conjugacy_BaseCases, SpThisStep;

OmegaThisStep := function (g, form, index, Label, P, Space, spaces: Orthogonal := true)
...
EvenSpaces := function (g, form, Labels: Orthogonal := false)
   ...
   index := #Labels;
   fct := Orthogonal select OmegaThisStep else SpThisStep;
...
OmegaEvenChooseBasis := function (g, form, L)
   V := QuadraticSpace (form);
...
Omega_Even_Conjugacy_BaseCases := function (h, form, type) 
   ...
   if type[1] eq "V" then 
      ...
      flag, C := Omega_WBeta_ConjugationMatrix (H, rep, form, g, 0);
   elif type[1] eq "W" and type[3] ne 0 then 
      alpha := type[3];
      n := type[2];
      rep := W_odd (n, q, alpha);
      form := OForm ("Omega", n, q, alpha: nmr := 1);
      flag, C := Omega_WBeta_ConjugationMatrix (H, rep, form, g, alpha);
   end if;
...
EvenConjugateBlock := function (block_g, block_h, form, type : Orthogonal := false)
   ...
   fct := Orthogonal select Omega_Even_Conjugacy_BaseCases else
...
OmegaEvenDecideConjugacy := function (G, g, h: InOmega := false, Verify := false)
   ...
   CA := OmegaEvenChooseBasis (g, new_form, label);
   new_g := CA * g * CA^-1;

   CB := OmegaEvenChooseBasis (h, new_form, label);
   new_h := CB * h * CB^-1;
