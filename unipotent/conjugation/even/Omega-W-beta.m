...
// handle W_n(beta) n odd beta different from 0 for orthogonal even char 

OmegaBasicSetup := function (rep, form, beta)
   d := Degree (rep);
   n := d div 2;
   k := n div 2;
   ...
   // if IsEven (n) then bound := k - 1; else bound := k; end if;
...
Omega_WBeta_ConjugationMatrix := function (IG, rep, form, h, beta: LIMIT := 100)
   d := Nrows (rep);
   n := d div 2;
   k := n div 2;
   ...
   Rels, F, G, H, K := OmegaBasicSetup (rep, form, beta);
   ...
   error "Failed to decide conjugacy"; 
end function;
