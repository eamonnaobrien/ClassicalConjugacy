...
import "odd-sp.m": SpQuad, SpPair;

// Sp even characteristic 
...
OrthogonalBlocks := function (r, I, q, a, form, type)
   ...
   return OrthogonalBlock (m, q, small, type);
end function;
...
AdditionalMatrix := function (d, q)
   MA := MatrixAlgebra (GF(q), 4);
   l := Identity (MA);
   x := MatrixAlgebra (GF(q), 2) ! [1,1,1,1];
   InsertBlock (~l, x, 1, 3);
   InsertBlock (~l, x, 3, 1);
   nr := d div 4;
   L := DiagonalJoin (<l: i in [1..nr]>);
   return sub<GL(d, q) | L>;
end function;
...
SpSSGenerators_Even := function (r, form, m, T, N, P, phi, tau)
   d := Nrows (r);
   q := #BaseRing (r);
...
// code to set up Q-group and class rep 

Sp_QSubgroup_Even := function (a, q, N)
...
Sp_ClassRep_Even := function (rep, form, m, T, q) 
   ...
   return GL(d, q) ! (P * rep * Transpose (P)), P * form * Transpose (P), S, N, P;
end function;
...
Sp_ConvertToSpecialRep_Even := function (g, form, T)
   ...
   r, form, Wt, N, CB := Sp_ClassRep_Even (g, form, m, T, q);
   r := GL(d, q) ! r;
   CB := GL(d, q) ! CB;
...
SpUnipotentCentraliserRep := function (G, g, T: 
   ...
   r, f, CB, m, W, N := Sp_ConvertToSpecialRep_Even (g, form, T);
   assert FixesForm (r, f);

   Q := Sp_QSubgroup_Even (W, q, N);
   ...
   Add := SpSSGenerators_Even (r, f, m, T, N, P, phi, tau);
