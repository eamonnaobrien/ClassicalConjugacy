...
// code for Sp even char and for Omega even char V(beta) 
...
// code for Sp and Omega even char V(beta) beta can be 0 or non-zero 

SpO_VBeta_Construct_ConjugateElement := function (g, rep, form: 
     Orthogonal := false, Verify := false)
   ...
   if Orthogonal then 
      P := PolynomialRing (F, k + 1);
   else 
      P := PolynomialRing (F, k);
   end if;
   ...
   if #C eq 1 then return false, _; end if;
...
SpO_VBeta_ConjugateElement := function (g, rep, form: 
        Orthogonal := false, Limit := 100)

   nmr := 0;
   repeat
      flag, CB := SpO_VBeta_Construct_ConjugateElement (g, rep, form: Orthogonal := Orthogonal);
      nmr +:= 1;
   until flag or nmr gt Limit;
   if nmr gt Limit then error "SpO_VBeta_ConjugateElement: Failed to decide conjugacy"; end if;
   if flag then
      return true, CB;
   else
      return false, _;
