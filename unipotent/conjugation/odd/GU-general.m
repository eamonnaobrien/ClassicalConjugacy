...
GU_Odd_VBetaConjugateElement := function (g) 
   ...
   assert exists(gamma) {x : x in F | x ne 0 and x + x^q eq F!-1};
...
SUParameters := function (G, g: rep := false, FullLabel := true)
   ...
   if FullLabel then 
      ...
      return <partition, alpha>, CB, Z1;
   else 
      return <partition, Params>, CB, Z1;
   end if;
end function;
...
SUDecideConjugacy := function (G, g, h)
   ...
   // try to construct a diagonal commuting element of determinant det^-1 
   ...
   Z := [z @@ phi: z in Z1];
   Z := sub< M | Z >;
   assert exists(z){z : z in Z | #Z eq Order (z)};
   Z := sub<M | z>;
