...
SpBasicSetup := function (rep, form, beta)
   ...
   GenerateRelation := function (F, PB, S, w1, w1_RHS, alpha1) 
      ...
      // alpha1 
      rel := &+[c[i] * c_rhs[d - i + 1]: i in [1..d]] + alpha1;
      return rel;
   end function;
   ...
   for i in [1..2*k] do 
      ...
      rel := GenerateRelation (F, PB, S, w1, alpha_RHS, alpha);
      Append (~Rels, rel);
   end for;
   for i in [1..2 * k] do 
      alpha_RHS := w1 * h_pols[i + 1] + x1 * k_pols[i + 1];
      rel := GenerateRelation (F, PB, S, x1, alpha_RHS, alpha);
      Append (~Rels, rel);
   end for;
   for i in [1..2*k + 1] do 
...
Sp_Construct_Wbeta_ConjugationMatrix := function (rep, form, h: LIMIT := 100) 
   ...
   repeat
      w := Random (V);
      x := Random (V);
   until sub<V | CS, w, x> eq V and 
   DotProduct (w, w - w * h) ne 0 and DotProduct (x, x - x * h) ne 0;
   ...
   Solns := Variety (I);
   if #Solns eq 0 then return false, 0; end if;
   ...
   for S in Solns do 
      ...
      if found then return found, CB; end if;
   end for;

   return false, -1;
end function;

Sp_WBeta_ConjugateElement := function (g, rep, form: LIMIT := 100) 
   nmr := 0;
   repeat 
      flag, CB := Sp_Construct_Wbeta_ConjugationMatrix (rep, form, g);
      nmr +:= 1;
   until flag or nmr gt LIMIT;
   if nmr gt LIMIT then error "Sp_WBeta_ConjugateElement: Failed to decide conjugacy"; end if;
   if flag then return flag, CB; else return false, _;  end if;
end function;
