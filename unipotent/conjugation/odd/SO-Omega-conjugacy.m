...
// labels for elements of Omega and SO
...
ModifyByCentre := function (Z, form, c, gens)
   ...
   if flag then return c * z; end if;

   error "Reached here in ModifyByCentre";
   nmr := 0;
   repeat 
      z := Random (Z);
      nmr +:= 1;
   until MySpinorNorm (c * z, form) eq 0;
   "Total number of random selections", nmr;
   return c * z;
end function;
