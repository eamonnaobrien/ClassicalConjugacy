...
// r is special rep for one block in Omega fixing form f and has parameter beta;
// u is conjugate in Omega to r; return conjugating element 

Omega_OneBlock_ConjugationMatrix := function (r, f, beta, u: Verify := false)
   ...
   A := Omega_OneBlock_SolveEquations (n, q, Alpha, alpha0, beta);
...
Omega_TwoBlocks_SolveEquations := function (Alpha, Beta, Gamma, k, q)
...
// r is special rep for two blocks in Omega fixing form f 
// u is conjugate in Omega to r; return conjugating element 

Omega_TwoBlocks_ConjugationMatrix := function (r, f, u: Verify := false)
   ...
   A0, A, B, C := Omega_TwoBlocks_SolveEquations (Alpha, Beta, Gamma, k, q);
...
OmegaOddDecomposition := function (g, form, L)
...
OmegaOrganiseBasis := function (U: VBlock := true, beta := 0)
   n := Dimension (U);
   k := n div 2;
...
OmegaChooseBasis := function (g, form, types, Beta) 
   spaces := OmegaOddDecomposition (g, form, [* *]);
   spaces := MyPullback (g, form, spaces);
   d := Nrows (form);
   F := BaseRing (form);
   CB := [* *];
   for i in [1..#spaces] do 
      C := OmegaOrganiseBasis (spaces[i]: VBlock := types[i], beta := Beta[i]);
      Append (~CB, C);
   end for;
   CB := [x : x in CB];
   CB := &cat (CB);
...
OmegaConjugateBlock := function (g, h, form: OneBlock := true)
   Beta := OrthogonalParameters (g, form);
   Beta_h := OrthogonalParameters (h, form);
   if Beta ne Beta_h then return false, _; end if;

   beta := [Beta[i][2]: i in [1..#Beta]];
   r := OmegaSetupSpecialRep (g, form);

   if OneBlock then 
      beta := beta[1];
      C1 := Omega_OneBlock_ConjugationMatrix (r, form, beta, g);
      C2 := Omega_OneBlock_ConjugationMatrix (r, form, beta, h);
   else
      C1 := Omega_TwoBlocks_ConjugationMatrix (r, form, g);
      C2 := Omega_TwoBlocks_ConjugationMatrix (r, form, h);
   end if;

   return true, C1^-1 * C2; 
end function;

OmegaConjugateBlocks := function (g, h, form, blocks, types)
   F := BaseRing (g);
   Conj := <>;
   for i in [1..#blocks] do
      ...
      if Nrows (J) gt 1 then 
         flag, one := OmegaConjugateBlock (block_g, block_h, J: OneBlock := types[i]);
      else 
...
OmegaSetupSpecialRep := function (g, form) 
   F := BaseRing (g);
   q := #F;
   L := OmegaOddDecomposition (g, form, [* *]);
   R := [* *];
   Dim := []; BV := [* *];
   for i in [1..#L] do 
      a := L[i][1]; fa := L[i][2];
      ...
      m := MySort (m);
      r, fr, cc, dd, ee := Omega_ClassRep (m, q, beta);
...
OrthogonalDecideConjugacy := function (G, g, h: InOmega := false) 
   ...
   new_rep, new_form, Dims, BetaV := OmegaSetupSpecialRep (g, form);
   ...
   CA, Sg := OmegaChooseBasis (g, new_form, types, BetaV);
   new_g := CA * g * CA^-1;

   CB := OmegaChooseBasis (h, new_form, types, BetaV);
   new_h := CB * h * CB^-1;

   flag, X := OmegaConjugateBlocks (new_g, new_h, new_form, Sg, types);
   if not flag then return false, _; end if;
