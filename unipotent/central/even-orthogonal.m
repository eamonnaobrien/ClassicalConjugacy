...
Omega_QSubgroup_Even := function (W, q, N, Q_form)
   weight := [W[i][1]: i in [1..#W]];
   ...
   // "MatB v blocks: vmi vmj vi vj for i > 1"
   B := Basis (F);
   gens := [];
   ...
   // "MatC v blocks: vmi vm1 v1 vi for i > 1"
   gens := [];
   B:= Basis (F);
   for a in [1..#v_list_minus] do
      i := v_list_minus[a];
   ...
   // "Mat14" v_m1 v_m1' v_m1'' v_1 v_1' v_1'' 
   gens := [];
   B := Basis (F);
   for a in [1..#v_list_minus] do 
      vmi := v_list_minus[a];
...
Omega_CentraliserDimension_Even := function (W)
...
// semisimple generators for unipotent element of Omega (d, q) where q is even
...
Case3 := function (s, d, q, beta) 
   ...
   // set up GO NOT Omega 
   G := IsometryGroup (V);
   // change to code to work with 2.25-7: ensure first generator is in Omega 
   // small := (a eq 1 and WittIndex (V) eq 0) select [Matrix (G.2), Matrix (G.1)]
//                                    else [Matrix (G.i) : i in [1..Ngens (G)]];
   
// original code 
   G := IsometryGroup (QuadraticSpace (form));
...
Omega_ClassRep_Even := function (rep, form, m, T, q) 
   // "Omega even characteristic: label function";
...
Omega_ConvertToSpecialRep_Even := function (g, form, T)
   d := Nrows (g);
   q := #BaseRing (g);
   J, CB, b := JordanForm (g);
   s := [b[i][2]: i in [1..#b]];
   m := [<x, #[y: y in s | y eq x]>: x in Set (s)];
   r, form, CB, m, Wt, N := Omega_ClassRep_Even (g, form, m, T, q);
   r := GL(d, q) ! r;
   CB := GL(d, q) ! CB;
...
OmegaSSGenerators_Even := function (r, P, phi, tau, N, N1, form, T, q: 
   OmegaOnly := false)
   ...
   if #Gens ge 2 then 
      l := #Gens[1]; 
      add  := [Gens[1][l] * Gens[j][#Gens[j]]: j in [2..#Gens]];
      rest := [[Gens[i][j]: j in [1..#Gens[i]]]: i in [1..#Gens]];
      Gens := &cat (rest) cat add;
   else
      rest := [[Gens[i][j]: j in [1..#Gens[i]]]: i in [1..#Gens]];
      Gens := &cat (rest);
   end if;
   ...
   mr := [Gens3[i]: i in index];
   Gens3 := [Gens3[i]: i in [1..#Gens3] | not (i in index)];
   assert forall{x: x in mr | Order ((r, x)) eq 1};

   if OmegaOnly eq false and #mr gt 0 then 
      // "3/3A Added additional generator here";
      Gens3 cat:= mr;
      OmegaOnly := true;
   end if;
   ...
   if #V_list gt 0 then 
      ...
      nmr := 0;
      for i in [1..#W_list] do
         rank := W_list[i];
         // if rank eq 1 then continue; end if;
         J := FindIndices (N, "V", (rank - 1) div 2);
         if #J gt 0 then continue; end if;
         ...
         for k in [1..#V_list] do 
            rank := V_list[k];
            LIndex  := FindIndices (N, "V", rank);
            for j in [1..#LIndex] do 
               I := LIndex[j]; 
               u1 := ExtractBlock (r, I, I);
               MA := MatrixAlgebra (GF(q), d);
               g := Identity (MA);
               u1 := MyInsert (g, u1, I, I);
               Append (~Gens3A, mr[nmr] * u1);
            end for;
         end for;
      end for;
   end if;
   Gens3A := [x: x in Gens3A];
   //  assert forall{x: x in Gens3A | Order ((r, x)) eq 1};
   //  "Case 3A: Number of gens is ", #Gens3A;
   ...
   if #V_list gt 0 then 
      ...
      add := [[gens[i] * gens[j]^-1: j in [i + 1..#gens]]: i in [1..#gens]];
      Gens := &cat (add);
      if OmegaOnly eq false then 
         // "*** 6 Added additional generator here";
         Append (~Gens, gens[1]);
         OmegaOnly := true;
      end if;
      ...
      u := phi (r);
      A := [v @ tau where _, v := IsConjugate (P, u, phi (r^l)): l in Gens];
      Gens6 := [Gens[i] * A[i]^-1: i in [1..#Gens]];
   else 
      Gens6 := [];
   end if;
   // "Case 6: Number of gens is ", #Gens6;

   H := sub<GL(d, q) | Gens1, Gens2, Gens3, Gens3A, Gens4, Gens5, Gens6>;
...
OrthogonalUnipotentCentraliserRep := function (G, g, f, T, type: 
   ...
   r, form, CB1, m, Wt, N := Omega_ConvertToSpecialRep_Even (g, f, T);

   Q_values := [form[i,i]: i in [1..Nrows (form)]];
   U := Omega_QSubgroup_Even (Wt, q, m, Q_values);
   assert IsUnipotent (U);
   Q := UnipotentMatrixGroup (U);
   P, phi, tau := PCPresentation (Q);
   // "# of generators for unipotent matrix group and for pc-group is ", Ngens (Q), NPCgens (P);
   ...
   qdim := Omega_CentraliserDimension_Even (Wt);
   assert Ilog (2, q) * qdim ge NPCgens (P); 
   // "qdim is ", qdim;

   Add := OmegaSSGenerators_Even (r, P, phi, tau, N, m, form, T, q: OmegaOnly := OmegaOnly);
...
// centraliser of g in GO or Omega
