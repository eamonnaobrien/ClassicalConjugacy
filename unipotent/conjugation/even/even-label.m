...
import "Omega-even-general.m": ClassSplitsInOmega_Even, OmegaEvenDecideConjugacy; 
 
// compute labels for elements of Sp and Omega in characteristic 2
...
OmegaOneStep := function (g, form, QForm, L)
   C, r := OneHomocyclicComponent (g, form);
...
// fairly "standard" description of element 
OmegaEvenApproximateLabel := function (g, QF)
   form := QF + Transpose (QF);
   L := [* *];
   a := g; f := form; qf := QF;
   repeat 
      V := QuadraticSpace (qf);
      C, L := OmegaOneStep (a, f, qf, L);
      assert C subset V;
      D := OrthogonalComplement (V, C);
...
SpO_EvenLabel := function (g, form: Orthogonal := false, Omega := false, 
   ...
   alpha := ChooseAlpha (q);
   if Orthogonal then 
      label := OmegaEvenApproximateLabel (g, form);
   else
   ...
   if Omega and ClassSplitsInOmega_Even (label) then
      ...
      flag, x := OmegaEvenDecideConjugacy (G, g, rep: InOmega := true);
